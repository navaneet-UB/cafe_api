...
app.config['SECRET_KEY'] = 'navaneet'
db = SQLAlchemy()
db.init_app(app)
...
def add():
    ...
    new_cafe = Cafe(name=name,location=location,map_url=map_url,img_url=img_url,has_sockets=has_sockets,has_toilet=has_toilet,can_take_calls=can_take_calls,seats=seats,coffee_price=coffee_price)
...
def update():
    ...
    if not cafe:
        ...
        db.session.commit()
    return "Table values are Updated"
